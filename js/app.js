...
async function updateInterface(virusKey) {
    ...
    try {
        ...
        currentTop3Data.forEach((candidate, index) => {
            ...
            // 5 Amino asitlik dizilimi bul
            const startPos = candidate.pos - 2;
            const endPos = candidate.pos + 2;
            const seqStr = fullSequence.substring(startPos - 1, endPos).split('').join('-');
            ...
            card.innerHTML = `
                ...
                <p style="font-size: 0.9rem; margin-bottom: 0.5rem;">Pozisyon Aralığı: <strong>${startPos} - ${endPos}</strong> (Merkez: ${candidate.pos})</p>
